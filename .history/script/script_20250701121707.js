...
function setDom() {
  switch (true) {//icon and background
    ...
    case state.toLowerCase() == "clear":
      ...
      case state.toLowerCase().includes("snow")||state.toLowerCase().includes("ice")||state.toLowerCase().includes("blizzard")||state.toLowerCase().includes("sleet")||state.toLowerCase().includes("freezing"):
      ...
      case state.toLowerCase().includes("rain")||state.toLowerCase().includes("drizzle"):
        stateIconDom.setAttribute("src", "media/rainy.png");
        background('rain');
        break

      case state.toLowerCase().includes("cloud")||state.toLowerCase().includes("overcast"):
  ...
  dayDateDom.innerHTML = `${date.getDate()} <small id="month">${months[date.getMonth()-1]}</small>`;
...
switch (true) {//icons
  ...
  case stateT.toLowerCase() == "clear":
    ...
    case stateT.toLowerCase().includes("snow")||stateT.toLowerCase().includes("ice")||stateT.toLowerCase().includes("blizzard")||stateT.toLowerCase().includes("sleet")||stateT.toLowerCase().includes("freezing"):
      futureIconSrcT = "media/snow.png";
      break



    case stateT.toLowerCase().includes("rain")||stateT.toLowerCase().includes("drizzle"):
      futureIconSrcT = "media/rainy.png";
      break

    case stateT.toLowerCase().includes("cloud")||stateT.toLowerCase().includes("overcast"):
      futureIconSrcT = "media/cloudy.png";
      break


    case stateT.toLowerCase().includes("fog")||stateT.toLowerCase().includes("mist"):
          ...
          <div class="left w-50">
              ...
              <div class="col-md-9">
                <div class="card-body h-100  d-flex flex-column justify-content-between pb-0">
                  <div class="card-title d-flex justify-content-between">
                    <small class="h6" id="dayNameT">${days[fDateT.getDay()]}</small>
                    <small class="h6" >Tomorrow</small>
                  </div>
                  <span class="card-text d-flex align-items-center justify-content-between">
                    <h1 id="tempT">
                    ${c?data.forecast.forecastday[1].day.avgtemp_c:data.forecast.forecastday[1].day.avgtemp_f}<sup>o</sup>
                    </h1>
                  </span>
                  <p class="card-text">
            ...
            <div class="row justify-content-end g-0">
              <div class="col-md-12">
                ...
                <div class="pie" style="--p:${data.forecast.forecastday[1].day.daily_chance_of_rain};--c:darkblue;--b:10px"> ${data.forecast.forecastday[1].day.daily_chance_of_rain}%</div>
...
for(let i=1 ; i<6 ; i++)
  {
    ...
    switch (true) {//icons
      ...
      case stat.toLowerCase() == "clear":
        ...
        case stat.toLowerCase().includes("snow")||stat.toLowerCase().includes("ice")||stat.toLowerCase().includes("blizzard")||stat.toLowerCase().includes("sleet")||stat.toLowerCase().includes("freezing"):
          futureIconSrc = "media/snow.png";
          break
  
  
  
        case stat.toLowerCase().includes("rain")||stat.toLowerCase().includes("drizzle"):
          futureIconSrc = "media/rainy.png";
          break
  
        case stat.toLowerCase().includes("cloud")||stat.toLowerCase().includes("overcast"):
          futureIconSrc = "media/cloudy.png";
          break
  
  
        case stat.toLowerCase().includes("fog")||stat.toLowerCase().includes("mist"):
    ...
    futureCards +=`
          ...
          <div class="card-footer d-flex justify-content-center align-items-center w-25  h2 m-0 p-0">
            ${c?data.forecast.forecastday[i+1].day.avgtemp_c:data.forecast.forecastday[i+1].day.avgtemp_f}<sup>o</sup>
          </div>
        </div>`
  }
...
async function getLocation() {
  ...
  userLocation = d.city;
   if (userLocation) {
      ...
      getCurrent("london"); // fallback لو الـ API مفشلش يرجع لندن
    }
  getCurrent(userLocation);
}
